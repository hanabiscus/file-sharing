  ...
  try {
    ...
    if (fileRecord.passwordHash) {
      ...
      if (!rateLimitResult.allowed) {
        ...
        return createErrorResponse(ErrorCode.INVALID_PASSWORD, message, origin, 429);
      }
      ...
      if (!isValidPassword) {
        ...
        const actualRemaining = rateLimitResult.remainingAttempts ? rateLimitResult.remainingAttempts - 1 : 0;
        const message = actualRemaining > 0
    ...
    return createErrorResponse(ErrorCode.STORAGE_ERROR, 'Failed to generate download link', origin);
  }
}

function createErrorResponse(code: ErrorCode, message: string, origin?: string, customStatusCode?: number): APIGatewayProxyResult {
  ...
  const statusCode = customStatusCode || 
                     (code === ErrorCode.FILE_NOT_FOUND ? 404 : 
                      code === ErrorCode.INVALID_PASSWORD ? 401 : 400);
