...
export class FileLairStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    const filesBucket = new s3.Bucket(this, 'FilesBucket', {
      bucketName: `filelair-files-${this.account}-${this.region}`,
      ...
      cors: [{
        ...
        allowedOrigins: [
          'https://d10in0kelu18m1.cloudfront.net',
          'http://localhost:5173' // Development only
        ],
        allowedHeaders: ['Content-Type', 'Content-Length', 'Authorization', 'x-amz-content-sha256', 'x-amz-date'],
        exposedHeaders: ['ETag'],
        maxAge: 3600
      }],
    ...
    const websiteBucket = new s3.Bucket(this, 'WebsiteBucket', {
      bucketName: `filelair-web-${this.account}-${this.region}`,
    ...
    const responseHeadersPolicy = new cloudfront.ResponseHeadersPolicy(this, 'SecurityHeadersPolicy', {
      securityHeadersBehavior: {
        ...
        contentSecurityPolicy: {
          contentSecurityPolicy: "default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval'; style-src 'self' 'unsafe-inline'; img-src 'self' data: https:; font-src 'self'; connect-src 'self' https://*.amazonaws.com; frame-ancestors 'none';",
          override: true
        }
      },
